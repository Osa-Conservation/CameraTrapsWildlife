...
SitesByPy0<-dep0 %>% 
  select(placename, longitude, latitude, subproject_name) %>% 
...
PuentesSites2<-PuentesSites %>% 
  select(Bridge_code, Longitude, Latitude, "2027_active") %>% 
...
pal <- colorFactor(
  palette = c("#639CA4FF", "#023743FF", "#72874EFF", "#476F84FF", "#A4BED5FF", "#BEB59CFF", "#DABD61FF"),  # specify your color pallete
...
LocationsMap <- leaflet(data = SitesByPy) %>%
  ...
  addCircleMarkers(
    ...
    radius = 5,
    fillOpacity = 0.8,
    color = ~pal(subproject_name),  # Color by subproject
    fillColor = ~pal(subproject_name),
    weight = 1,
    opacity = 1,
    popup = ~paste(
