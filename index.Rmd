...
for(i in 1:nrow(tmp))
{
  if(ymd(substr(tmp$start_date[i],1,10))!=ymd(substr(tmp$end_date[i],1,10)))
  {
    daily_lookup[[i]] <- data.frame(
      "date"=seq(ymd(substr(tmp$start_date[i],1,10)),
                 ymd(substr(tmp$end_date[i],1,10)), by="days"),
      "placename"=tmp$placename[i])
  }
}
...
dep_short<- dep_short[duplicated(dep_short)==F,]
...
sppSummary0 <- long_obs %>%                   # Take the long observation data frame `long_obs` 
...
sp_summary <- left_join(sp_summary, OccuSummary)
